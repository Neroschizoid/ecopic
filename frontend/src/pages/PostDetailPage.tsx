...
export default function PostDetailPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      ...
      <button
        onClick={() => navigate(-1)}
        className="flex items-center text-gray-600 hover:text-leaf-600 mb-6 transition-colors"
      >
        ...
        <div className="p-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              <div className="w-12 h-12 bg-leaf-100 rounded-full flex items-center justify-center mr-4">
                <span className="text-leaf-600 font-semibold text-lg">
                  {post.user?.username?.charAt(0).toUpperCase() || 'U'}
                </span>
              </div>
              <div>
                <p className="font-semibold text-gray-900 text-lg">{post.user?.username || 'Anonymous'}</p>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="px-4 py-2 bg-leaf-100 text-leaf-700 rounded-full text-sm font-medium"
              >
                #{tag}
              </span>
            ))}
          ...
          <div className="flex items-center justify-between pt-6 border-t border-gray-200">
            ...
            <div className="flex items-center">
              <span className="text-3xl font-bold text-leaf-600">+{post.points}</span>
