...
export default function RedeemPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setUserCredits(userData.carbon_credits)
    } catch (error) {
        ...
        return prevCart.map(item =>
          item.reward.id === reward.id
            ? {
                ...
                total_points: reward.points_required * (item.quantity + 1)
              }
            : item
        )
      }
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-8 flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600">Exchange your carbon credits for eco-friendly products</p>
        </div>
        <button
      ...
      <div className="mt-4 p-4 bg-leaf-50 rounded-lg mb-8">
        <span className="text-lg font-semibold text-leaf-700">🌿 Your Credits: {userCredits.toLocaleString()} points</span>
      </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {rewards.map(reward => (
                <div key={reward.id} className="card hover:shadow-lg transition-shadow">
                  <div className="h-48 bg-gradient-to-br from-leaf-100 to-leaf-50 rounded-lg mb-4 flex items-center justify-center">
                    {reward.image_url ? (
                      <img
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-lg font-bold text-leaf-600">{reward.points_required} credits</span>
                    <button
              ...
              {cart.length === 0 ? (
                ...
                <>
                  <div className="space-y-4 mb-6 max-h-96 overflow-y-auto">
                    {cart.map(item => (
                      <div key={item.reward.id} className="border border-gray-200 rounded-lg p-3">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="font-semibold text-sm text-gray-900 flex-1">{item.reward.name}</h4>
                          <button
                        ...
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs text-gray-600">{item.reward.points_required} pts each</span>
                          <span className="font-semibold text-leaf-600">{item.total_points} pts</span>
                        </div>
                  ...
                  <div className="border-t border-gray-200 pt-4">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="text-2xl font-bold text-leaf-600">{cartTotal.toLocaleString()}</span>
                    </div>
