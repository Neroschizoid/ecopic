...
export default function UserProfilePage() {
  ...
  const fetchUserPosts = async () => {
    try {
      ...
      const posts = await postsAPI.getPosts({ limit: 20 }) // This should be filtered by user on backend
        ...
        <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex-1">
              {!isEditing ? (
                <>
                  ...
                  <p className="text-leaf-600 font-semibold mt-1">
                    🌿 Total Credits: {profileUser.carbon_credits.toLocaleString()}
                  </p>
                  <p className="text-gray-500 text-sm mt-1">
                    Member since {formatDate(profileUser.created_at)}
                  </p>
                  <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                    <span>{profileUser.stats.posts_count} posts</span>
                    <span>{profileUser.stats.total_points_earned} points earned</span>
                  </div>
                </>
              ) : (
                <form onSubmit={handleSubmit(onUpdateProfile)} className="space-y-4">
                  <div>
                    <input
                      ...
                      className="text-2xl font-bold bg-transparent border-b-2 border-gray-300 focus:border-leaf-600 outline-none"
                  ...
                  <div>
                    <textarea
                      ...
                      rows={3}
                      className="w-full p-2 border border-gray-300 rounded focus:border-leaf-600 outline-none resize-none"
          ...
          {isOwnProfile && !isEditing && isAuthenticated && (
            <button
              ...
              className="p-2 text-gray-600 hover:text-leaf-600 transition-colors"
              title="Edit profile"
            >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-wrap gap-1">
                    {post.tags.slice(0, 2).map((tag) => (
                      <span key={tag} className="text-xs bg-leaf-100 text-leaf-700 px-2 py-1 rounded-full">
                        {tag}
                      </span>
                    ))}
                  </div>
