  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mb-6">Posts from people and tags you follow</p>
      </div>

      {/* Followed Tags Display */}
      {followedTags.length > 0 && (
        <div className="mb-8 p-4 bg-leaf-50 rounded-lg border border-leaf-200">
          <p className="text-sm font-semibold text-gray-700 mb-3">Following {followedTags.length} tag{followedTags.length !== 1 ? 's' : ''}</p>
          <div className="flex flex-wrap gap-2">
            {followedTags.map((tag) => (
              <button
                key={tag}
                onClick={() => handleUnfollowTag(tag)}
                className="flex items-center gap-2 px-3 py-1 text-sm bg-white border border-leaf-200 rounded-full hover:bg-red-50 transition-colors"
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Start Following to Build Your Feed</h3>
          <p className="text-gray-600 mb-6">
            Follow users and tags to see their eco-action posts in your personalized feed.
          </p>
          <button
            onClick={() => navigate('/feed')}
            className="inline-flex items-center px-6 py-2 bg-leaf-600 text-white rounded-lg hover:bg-leaf-700 transition-colors"
          >
        ...
        <div className="text-center py-12">
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No New Posts Yet</h3>
          <p className="text-gray-600 mb-6">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">Try following different users or tags.</p>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {posts.map((post) => (
